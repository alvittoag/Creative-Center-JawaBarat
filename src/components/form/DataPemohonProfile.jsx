...
export default function DataPemohonProfile({ setActive, data }) {
  const formik = useFormik({
    initialValues: {
      ...
      kecamatanPemohon: data?.kecamatanPemohon || "",
      kelurahanPemohon: data?.kelurahanPemohon || "",
      kabPemohon: data?.kabPemohon || "",
      statusPemohon: data?.statusPemohon || "Pelajar/Mahasiswa",
      noPemohon: data?.noPemohon || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (data?.id) {
        ...
        const { error } = await supabase
          .from("pemohon")
          .insert({ ...formik.values, ktp: upload?.ktp, user_id: user.id })
          .select();
    ...
    <div className="flex flex-col bg-[#fcfcfc] w-[40%] m-7 rounded-lg border justify-center">
      ...
      {data?.ktp && (
        <div className="w-full max-w-xs ml-[85px] mt-4 text-black ">
          <a target="_blank" className="underline" href={data.ktp}>
            Lihat KTP Anda
          </a>
        </div>
      )}
      ...
      <div className="grid place-content-end px-14 my-5">
        ...
        >
          {loadingUpload ? "Loading..." : data ? "Update" : "Next"}
        </button>
      </div>
    </div>
  );
